...
function render(record, preview) {
  ...
  const galleryHtml = gallery.length ? `<section class="section section-soft"><div class="wrap"><div class="section-head"><p class="eyebrow">Image Gallery</p><h2>${isCaseStudy ? "Design, production and quality review." : "Completed work and technical views."}</h2></div><div class="case-library-gallery">${gallery.map((image) => `<figure><a href="${escapeHtml(image.url)}" target="_blank" rel="noopener"><img src="${escapeHtml(image.url)}" alt="${escapeHtml(image.caption || `${item.title} ${image.imageType}`)}" loading="lazy"></a><figcaption><span>${escapeHtml(image.imageType)}</span>${image.caption ? escapeHtml(image.caption) : ""}</figcaption></figure>`).join("")}</div></div></section>` : "";
...
<link rel="stylesheet" href="/assets/site.css?v=manager-final-1"></head><body>${previewBanner}
