...
function renderFeaturedCase($, section, cases, label, key) {
  const container = $(`[data-featured-case='${key}']`).first();
  if (!container.length || !section?.caseId) return;
  const item = cases.find((entry) => entry.id === section.caseId);
  if (!item?.coverImage) return;
  const card = `<div class="section-head"><p class="eyebrow">${escapeHtml(label)}</p><h2>${escapeHtml(section.heading || label)}</h2>${section.description ? `<p class="copy">${escapeHtml(section.description)}</p>` : ""}</div><a class="managed-featured-case" href="/cases/${encodeURIComponent(item.slug)}"><img src="${escapeHtml(item.coverImage.url)}" alt="${escapeHtml(item.title)}" width="1200" height="900" loading="lazy" decoding="async"><div><p class="case-category">${escapeHtml(item.category)}</p><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.summary || item.shortNote || "View technical case details.")}</p><span class="text-link">View Case</span></div></a>`;
  container.removeAttr("hidden");
  container.html(container.is("section") ? `<div class="wrap">${card}</div>` : card);
}

function linkFullArchOptions($, pageConfig, cases) {
  (pageConfig?.restorationOptions || []).forEach((option, index) => {
    if (!option?.caseId) return;
    const item = cases.find((entry) => entry.id === option.caseId);
    if (item?.slug) $(`[data-option-case='${index}']`).attr("href", `/cases/${encodeURIComponent(item.slug)}`);
  });
}
...
function renderPublicPage(page, data, options = {}) {
  ...
  if (page === "implant") {
    applyManagedPage($, data.pages.implant);
    renderFeaturedCase($, data.pages.implant?.featuredWork, data.publishedCases || [], "Featured Implant Work", "implant");
  }
  if (page === "fullArch") {
    applyManagedPage($, data.pages.fullArch);
    renderFeaturedCase($, data.pages.fullArch?.featuredCase, data.publishedCases || [], "Featured Full-Arch Case", "fullArch");
    linkFullArchOptions($, data.pages.fullArch, data.publishedCases || []);
  }
