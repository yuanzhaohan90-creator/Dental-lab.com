...
function isSafeSvg(bytes, size) {
  if ((Number(size) || bytes.length) > 1024 * 1024) return false;
  const text = Buffer.from(bytes).toString("utf8").replace(/^\uFEFF/, "").trim();
  if (!/^<svg\b/i.test(text) && !/^<\?xml[\s\S]*?<svg\b/i.test(text)) return false;
  return !/<script\b|<foreignObject\b|\son\w+\s*=|\b(?:href|xlink:href)\s*=\s*["'](?:https?:|\/\/|data:)|url\s*\(|<!ENTITY/i.test(text);
}
...
function validateMediaFile({ filename, contentType, size, bytes, codecHint = "" }) {
  ...
  if (IMAGE_EXTENSIONS.has(extension) || mime.startsWith("image/")) {
    const imageFormat = isJpeg(data) ? "JPEG" : isPng(data) ? "PNG" : isWebp(data) ? "WebP" : (extension === "svg" || mime === "image/svg+xml") && isSafeSvg(data, actualSize) ? "SVG" : "";
    if (!imageFormat) throw Object.assign(new Error("Unable to read this image file."), { statusCode: 400, code: "INVALID_IMAGE" });
    const expected = { JPEG: "image/jpeg", PNG: "image/png", WebP: "image/webp", SVG: "image/svg+xml" }[imageFormat];
