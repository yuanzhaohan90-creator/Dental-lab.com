...
function backend() {
  return String(process.env.STORAGE_BACKEND || "vercel-blob").trim().toLowerCase();
}
...
async function storageClient() {
  ...
  vercelClient ||= import("@vercel/blob");
  return vercelClient;
}

function resetStorageClientsForTests() {
  r2Client = undefined;
  vercelClient = undefined;
}

module.exports = { isR2Backend, resetStorageClientsForTests, storageClient };
