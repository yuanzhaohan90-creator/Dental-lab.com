...
const DEFAULT_HOMEPAGE = {
  ...
  workflow: {
    ...
    ].map((item) => ({ ...item, media: mediaDefault("", `${item.label} workflow`) }))
  },
  finalCta: {
...
function scanMediaReferences(value, mediaId, source, variant, path = "", usage = []) {
  ...
  for (const key of ["mediaId", "posterMediaId", "imageMediaId"]) {
    if (value[key] !== mediaId) continue;
    const fullPath = path ? `${path}.${key}` : key;
    const fallbackAvailable = key !== "mediaId" || Boolean(value.fallbackPath || value.imagePath);
    usage.push({
...
async function trashMedia(record) {
  const dates = trashDates();
  return saveMedia({ ...record, trashedAt: dates.deletedAt, trashExpiresAt: dates.expiresAt, originalLocation: record.category || "Other" });
}
