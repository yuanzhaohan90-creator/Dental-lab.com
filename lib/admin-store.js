...
const { storageClient } = require("./object-store");
...
const DEFAULT_PAGE_CONFIGS = {
  ...
  fullArch: {
    ...
    framework: {
      ...
      destination: "/full-arch-all-on-x"
    },
    workflow: {
...
const OBSOLETE_REFERENCE_DESTINATIONS = new Set([
  "/cases/dual-arch-all-on-x",
  "/full-arch/zirconia-titanium-workflow",
  "/full-arch/immediate-load-pmma",
  "/full-arch/photogrammetry-workflow",
  "/full-arch/fp1-full-arch-zirconia",
  "/full-arch/complex-zygomatic-workflow"
]);

function normalizeDestination(value, fallback = "") {
  const destination = cleanText(value || fallback, 500);
  if (!destination) return "";
  if (OBSOLETE_REFERENCE_DESTINATIONS.has(destination)) return cleanText(fallback, 500);
