...
const MEDIA_EXTENSIONS = new Set(["jpg", "jpeg", "png", "webp", "mp4"]);
...
async function storeMediaFile(file, metadata) {
  ...
  if (!MEDIA_EXTENSIONS.has(ext) || !/^(image\/(jpeg|png|webp)|video\/mp4)$/.test(file.contentType)) {
    throw Object.assign(new Error("Only JPG, PNG, WebP and MP4 files are accepted."), { statusCode: 400 });
  }
...
async function storeClientMedia(blobResult, metadata = {}) {
  const pathname = cleanText(blobResult?.pathname, 500);
  const contentType = cleanText(blobResult?.contentType, 100).toLowerCase();
  const ext = mediaExtension(pathname);
  if (!pathname.startsWith(MEDIA_FILE_PREFIX) || !MEDIA_EXTENSIONS.has(ext) || !/^(image\/(jpeg|png|webp)|video\/mp4)$/.test(contentType)) {
    throw Object.assign(new Error("Uploaded media is not valid."), { statusCode: 400 });
  }
  const id = makeAdminId("MEDIA");
  const now = new Date().toISOString();
  const normalized = normalizeMediaRecord(metadata);
