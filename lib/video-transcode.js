...
let runtimeFfmpegPath = "";

function executablePath(command) {
  if (command !== ffmpegPath || process.platform === "win32") return command;
  if (runtimeFfmpegPath && fs.existsSync(runtimeFfmpegPath)) return runtimeFfmpegPath;
  const target = path.join(os.tmpdir(), "yzh-ffmpeg");
  if (!fs.existsSync(target)) fs.copyFileSync(ffmpegPath, target);
  fs.chmodSync(target, 0o755);
  runtimeFfmpegPath = target;
  return target;
}
...
async function blobClient(customClient) {
  return customClient || storageClient();
}
